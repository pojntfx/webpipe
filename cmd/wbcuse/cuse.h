...
static void fioc_do_rw(fuse_req_t req, void *addr, const void *in_buf,
  ...
  out_iov[0].iov_base = addr + offsetof(struct fioc_rw_arg, prev_size);
  out_iov[0].iov_len = sizeof(arg->prev_size);

  out_iov[1].iov_base = addr + offsetof(struct fioc_rw_arg, new_size);
...
static void cusexmp_ioctl(fuse_req_t req, int cmd, void *arg,
  ...
  case FIOC_GET_SIZE:
    if (!out_bufsz) {
      ...
      fuse_reply_ioctl(req, 0, &cusexmp_size, sizeof(cusexmp_size));
    break;
...
#define CUSEXMP_OPT(t, p)                                                      \
  { t, offsetof(struct cusexmp_param, p), 1 }
